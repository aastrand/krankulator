...
def main():
    ...
    # NOP with immediate, 2 bytes 2 cycles
    opcodes["NOP_82"] = ("82", 2, 2, "NOP #imm (Unofficial opcode)")
    opcodes["NOP_89"] = ("89", 2, 2, "NOP #imm (Unofficial opcode)")
    opcodes["NOP_C2"] = ("c2", 2, 2, "NOP #imm (Unofficial opcode)")
    opcodes["NOP_E2"] = ("e2", 2, 2, "NOP #imm (Unofficial opcode)")

    # ANC = AND #imm + copy N to C
    opcodes["ANC_0B"] = ("0b", 2, 2, "ANC = AND + copy N to C (Unofficial opcode)")
    opcodes["ANC_2B"] = ("2b", 2, 2, "ANC = AND + copy N to C (Unofficial opcode)")

    # NOP abs, 3 bytes 4 cycles
    opcodes["NOP_0C"] = ("0c", 3, 4, "NOP abs (Unofficial opcode)")

    # NOP zp, 2 bytes 3 cycles
    opcodes["NOP_04"] = ("04", 2, 3, "NOP zp (Unofficial opcode)")
    opcodes["NOP_44"] = ("44", 2, 3, "NOP zp (Unofficial opcode)")
    opcodes["NOP_64"] = ("64", 2, 3, "NOP zp (Unofficial opcode)")

    # NOP #imm, 2 bytes 2 cycles
    opcodes["NOP_80"] = ("80", 2, 2, "NOP #imm (Unofficial opcode)")

    # NOP implied, 1 byte 2 cycles
    opcodes["NOP_1A"] = ("1a", 1, 2, "NOP implied (Unofficial opcode)")
    opcodes["NOP_3A"] = ("3a", 1, 2, "NOP implied (Unofficial opcode)")
    opcodes["NOP_5A"] = ("5a", 1, 2, "NOP implied (Unofficial opcode)")
    opcodes["NOP_7A"] = ("7a", 1, 2, "NOP implied (Unofficial opcode)")
    opcodes["NOP_DA"] = ("da", 1, 2, "NOP implied (Unofficial opcode)")
    opcodes["NOP_FA"] = ("fa", 1, 2, "NOP implied (Unofficial opcode)")

    # ALR = AND #imm + LSR A
    opcodes["ALR_IMM"] = ("4b", 2, 2, "ALR = AND + LSR (Unofficial opcode)")

    # ARR = AND #imm + ROR A
    opcodes["ARR_IMM"] = ("6b", 2, 2, "ARR = AND + ROR (Unofficial opcode)")

    # XAA = TXA + AND #imm (unstable)
    opcodes["XAA_IMM"] = ("8b", 2, 2, "XAA = TXA + AND (Unofficial opcode, unstable)")

    # LAX #imm (unstable)
    opcodes["LAX_IMM"] = ("ab", 2, 2, "LAX #imm (Unofficial opcode, unstable)")

    # SBX = (A AND X) - #imm
    opcodes["SBX_IMM"] = ("cb", 2, 2, "SBX = (A AND X) - imm (Unofficial opcode)")

    # SHA = store A AND X AND (high+1)
    opcodes["SHA_INY"] = ("93", 2, 6, "SHA = store A AND X AND (high+1) (Unofficial opcode)")
    opcodes["SHA_ABY"] = ("9f", 3, 5, "SHA = store A AND X AND (high+1) (Unofficial opcode)")

    # TAS = S = A AND X, store S AND (high+1)
    opcodes["TAS_ABY"] = ("9b", 3, 5, "TAS = S = A AND X, store S AND (high+1) (Unofficial opcode)")

    # SHY = store Y AND (high+1)
    opcodes["SHY_ABX"] = ("9c", 3, 5, "SHY = store Y AND (high+1) (Unofficial opcode)")

    # SHX = store X AND (high+1)
    opcodes["SHX_ABY"] = ("9e", 3, 5, "SHX = store X AND (high+1) (Unofficial opcode)")

    # LAS = LDA/TSX (S AND mem)
    opcodes["LAS_ABY"] = ("bb", 3, 4, "LAS = LDA/TSX (S AND mem) (Unofficial opcode)")
...
impl Lookup {
    ...
    for key, (opcode, size, time, comment) in opcodes.items():
        mode_suffix = MODE_OVERRIDES.get(key, key[4:] if len(key) > 3 else "NA")
        mode = "ADDR_MODE_" + mode_suffix if mode_suffix in ADDRESSING_MODES else "ADDR_MODE_NA"
        basename = key[:3]
        suffix = mode[-3:]
        is_indexed = suffix in PAGE_BOUNDARY_PENALTY_MODES
        is_rmw = basename in RMW_BASENAMES
        is_store = basename in ("STA", "SAX", "SHA", "SHY", "SHX", "TAS")
