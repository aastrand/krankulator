...
const RAM_SIZE: usize = 0x1400; // 5KB battery-backed at $6000-$73FF
...
impl TaitoX1017Mapper {
    ...
    fn ram_segment_enabled(&self, addr: u16) -> bool {
        match addr {
            0x6000..=0x67FF => self.ram_permission[0] == 0xCA,
            0x6800..=0x6FFF => self.ram_permission[1] == 0x69,
            0x7000..=0x73FF => self.ram_permission[2] == 0x84,
            _ => false,
        }
    }
    ...
    fn chr_1k_index(&self, addr: u16) -> usize {
        ...
        // chr_mode swaps the 2x2KB and 4x1KB halves between $0000 and $1000
        let effective = if self.chr_mode != 0 { slot ^ 4 } else { slot };
    ...
    fn write_register(&mut self, addr: u16, value: u8) {
        match addr {
            ...
            0x7EF6 => {
                ...
                self.chr_mode = (value & 0x02) >> 1;
            }
            0x7EF7..=0x7EF9 => self.ram_permission[(addr - 0x7EF7) as usize] = value,
            0x7EFA..=0x7EFC => self.prg_regs[(addr - 0x7EFA) as usize] = value >> 2,
            _ => {}
        }
    }
}

impl MemoryMapper for TaitoX1017Mapper {
    fn cpu_read(&mut self, addr: u16) -> u8 {
        match addr {
            0x0000..=0x1FFF => self.cpu_ram[(addr & 0x7FF) as usize],
            0x6000..=0x73FF if self.ram_segment_enabled(addr) => self.ram[(addr - 0x6000) as usize],
    ...
    fn cpu_write(&mut self, addr: u16, value: u8) {
        match addr {
            ...
            0x6000..=0x73FF => {
                if self.ram_segment_enabled(addr) {
    ...
    fn save_state(&self, w: &mut SavestateWriter) {
        ...
        w.write_bytes(&self.ram_permission);
    ...
    fn load_state(&mut self, r: &mut SavestateReader) -> std::io::Result<()> {
        ...
        r.read_bytes_into(&mut self.ram_permission)?;
...
mod tests {
    ...
    fn make_mapper() -> TaitoX1017Mapper {
        ...
        TaitoX1017Mapper::new(0, prg, chr, None)
    }

    #[test]
    fn test_prg_banking_shifted() {
        let mut m = make_mapper();

        assert_eq!(m.cpu_read(0xE000), 7); // fixed last
        m.cpu_write(0x7EFA, 3 << 2);
        m.cpu_write(0x7EFB, 4 << 2);
        m.cpu_write(0x7EFC, 5 << 2);
    ...
    #[test]
    fn test_chr_mode_swap() {
        let mut m = make_mapper();

        m.cpu_write(0x7EF0, 4); // 2KB regs
        m.cpu_write(0x7EF2, 10); // 1KB regs
    ...
    fn test_ram_segments() {
        ...
        // Other segments still locked
        m.cpu_write(0x6800, 0x22);
        assert_eq!(m.cpu_read(0x6800), 0);

        m.cpu_write(0x7EF8, 0x69);
        m.cpu_write(0x6800, 0x22);
        assert_eq!(m.cpu_read(0x6800), 0x22);

        m.cpu_write(0x7EF9, 0x84);
        m.cpu_write(0x7000, 0x33);
        assert_eq!(m.cpu_read(0x7000), 0x33);
    }
}
